...
db.artworks.find().limit(2).pretty();

// .skip()

db.artworks.find().limit(2).skip(1).pretty();
...
// 32 MB limit: https://docs.mongodb.com/manual/reference/limits/#Sort-Operations
// check: https://docs.mongodb.com/manual/reference/method/db.collection.aggregate/#method-aggregate-allowdiskuse
db.artworks.find({}, {Title:1, Date:1}).sort({Date:-1});
...
db.artworks.find({Date: {$type: "string"}}).count();
...
db.artworks.find({_id: {$regex: '^N'}}).explain();
...
// $sort
// show them some problems with data
...
db.artists.aggregate([
    {
        $project: {_id:0, mda:1, activePlaceCount:1, totalWorks:1}
    },
    {
