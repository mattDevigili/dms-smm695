...
// using load:
load("/Your/Path/To/artworks.txt")
...
//to load locally
//data retrieved from http://jmcauley.ucsd.edu/data/amazon/
...
db.artists.find({movNames: {$size:3}}).count()
...
db.artists.find(
    {"movements": 
        {$elemMatch: {"id" : {$gt: 3}, "name": "Fancy Picture"}}
    },
    {_id:0, mda:1, movements:1}
).pretty()
