...
ALTER TABLE people.person ADD CONSTRAINT car_fk FOREIGN KEY (car_id) REFERENCES people.car(id);
-- Modify data in our person table
UPDATE people.person SET car_id = 3 WHERE id = 1;
UPDATE people.person SET car_id = 2 WHERE id = 2;
UPDATE people.person SET car_id = 1 WHERE id = 3; -- Try to see what does it happen if you insert a car that does not exist: UPDATE people.person SET car_id = 1 WHERE id = 4;
...
TO '/PATH/TO/YOUR/FOLDER/cross_join.csv' DELIMITER ',' CSV HEADER; -- remember to insert the path to a folder that PgAmin can access!